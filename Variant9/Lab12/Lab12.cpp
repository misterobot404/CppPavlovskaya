...
Это означает, что объект класса Vect используется как элемент класса Queue.
...
int main()
{
	...
	try
	{
		...
		cout << "Первый элемент очереди: " << *queueInt.begin() << endl;
		cout << "Последний элемент очереди: " << *(queueInt.end()-1) << endl;

		cout << "Освобождение очереди..." << endl;
		while (!queueInt.empty())
		{		
			cout << "Освобождаемый элемент: " << *queueInt.begin() << endl;
			queueInt.pop_begin();
		}

		cout << "Попытка удалить ещё один элемент из очереди.." << endl;
		// исключение т.к. очередь уже пуста
		queueInt.pop_begin();
	}
	...
	try
	{
		...
		cout << "Первый элемент очереди: " << *queueDouble.begin() << endl;
		cout << "Последний элемент очереди: " << *(queueDouble.end()-1) << endl;

		cout << "Освобождение очереди..." << endl;
		while (!queueDouble.empty())
		{
			cout << "Освобождаемый элемент: " << *queueDouble.begin() << endl;
	...
	try
	{
		...
		cout << "Первый элемент очереди: " << *queueString.begin() << endl;
		cout << "Последний элемент очереди: " << *(queueString.end()-1) << endl;

		cout << "Освобождение очереди..." << endl;
		while (!queueString.empty())
		{
			cout << "Освобождаемый элемент: " << *queueString.begin() << endl;
